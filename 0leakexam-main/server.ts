...
async function startServer() {
  ...
  app.post('/api/question-papers/extract', authenticateToken, requireRole(['EXAM_MANAGER', 'ORG_OWNER']), async (req: Request, res: Response) => {
    try {
      const { paper_text, file_name, file_data, subject, category, draft_count } = req.body;

      if (category === 'University Exam') {
        const fileCount = Number(draft_count ?? 0);
        if (!Number.isNaN(fileCount) && fileCount > 3) {
          return res.status(400).json({ error: 'Maximum 3 PDF drafts allowed for University Exam.' });
        }
      }
