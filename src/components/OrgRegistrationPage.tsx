...
export const OrgRegistrationPage: React.FC<OrgRegistrationPageProps> = ({
  ...
  const orgTypeOptions = [
    { value: 'Government Examination Authority', label: 'Government Examination Authority' },
    { value: 'University', label: 'University' },
    { value: 'College / Educational Institution', label: 'College / Educational Institution' },
    { value: 'Examination Board', label: 'Examination Board' },
    { value: 'Entrance Examination Authority / Entrance Test Cell', label: 'Entrance Examination Authority / Entrance Test Cell' },
    { value: 'Company / Private Organization', label: 'Company / Private Organization' },
    { value: 'Other', label: 'Other' },
  ];

  const orgTypeDescriptions: Record<string, string> = {
    'Government Examination Authority': 'Examples: UPSC, MPSC, SSC, Banking, Railway, Defence and other government recruitment examinations.',
    University: 'Examples: University semester, theory, practical, end-semester and university-level examinations.',
    'College / Educational Institution': 'Examples: Internal exams, semester exams, unit tests and institutional examinations.',
    'Examination Board': 'Examples: Board examinations, secondary/higher-secondary examinations and certification examinations.',
    'Entrance Examination Authority / Entrance Test Cell': 'Examples: CET Cell, TCET-type examinations, centralized entrance examinations and admission/entrance tests.',
    'Company / Private Organization': 'Examples: Recruitment tests, aptitude tests, technical assessments, certification exams and private competitive examinations.',
    Other: 'For organizations that do not fit any of the above categories.',
  };

  const registrationFieldConfig: Record<string, { label: string; placeholder: string; helper: string }> = {
    'Government Examination Authority': { label: 'Registration Number / ID *', placeholder: 'e.g. UPSC/2026/REG-12345', helper: 'Government examination authority verification uses the official registration source.' },
    University: { label: 'University Registration Number / Authorization ID *', placeholder: 'e.g. UNIV-DEL-2026-94', helper: 'University verification uses the official institutional authority.' },
    'College / Educational Institution': { label: 'Institution Registration Number / Affiliation ID *', placeholder: 'e.g. COL-2026-9876', helper: 'College verification uses the institution registration authority.' },
    'Examination Board': { label: 'Board Registration Number / ID *', placeholder: 'e.g. BOARD-2026-0145', helper: 'Examination board verification uses the official board registration source.' },
    'Entrance Examination Authority / Entrance Test Cell': { label: 'Entrance Authority Registration Number / ID *', placeholder: 'e.g. CET-2026-7890', helper: 'Entrance authority verification uses the official registration authority.' },
    'Company / Private Organization': { label: 'Company Registration Number / ID *', placeholder: 'e.g. U72900DL2020PTC123456', helper: 'Company verification uses the official registration source.' },
    Other: { label: 'Official Registration Number / ID *', placeholder: 'Enter the official registration number or legal ID', helper: 'A recognized official verification source is required before activation.' },
  };
  ...
  const nextStep = () => {
    setErrorMessage(null);
    if (step === 1) {
      if (!orgType || !orgName || !regNumber || !officialEmail) {
        setErrorMessage('Please complete all mandatory institutional details, including the Organization Type.');
        return;
      }
    } else if (step === 2) {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-3xl mx-auto w-full">
        ...
        <div className="flex justify-center mb-4">
          <ZeroLeakLogo size="md" variant="lockup" />
        </div>
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="mt-2 p-2.5 rounded-lg bg-slate-50 border border-slate-200 text-[10px] text-slate-600 leading-relaxed">
                    <span className="font-bold text-slate-800 block mb-1">{orgType}</span>
                    {orgTypeDescriptions[orgType] || 'Select the most suitable organization type.'}
                  </div>
                </div>

                <div>
                  <label className="block text-slate-700 font-bold mb-1">
                    {registrationFieldConfig[orgType]?.label || 'Registration Number / ID *'}
                  </label>
                  <input
                    type="text"
                    value={regNumber}
                    onChange={e => setRegNumber(e.target.value)}
                    placeholder={registrationFieldConfig[orgType]?.placeholder || 'Enter registration number'}
                    required
                    className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">{registrationFieldConfig[orgType]?.helper}</p>
                </div>
