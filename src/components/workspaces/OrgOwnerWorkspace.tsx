...
export const OrgOwnerWorkspace: React.FC<OrgOwnerWorkspaceProps> = ({
  ...
  const handleRegisterDevice = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.registerDevice({
        device_name: deviceName || 'Secured Administrative Workstation',
  ...
  const handleTrustDevice = async (deviceId: string) => {
    ...
    setProcessingDeviceId(deviceId);
    try {
      const res = await api.trustDevice(deviceId);
      setStatusMessage({ type: 'success', text: res.message });
      await loadData();
    } catch (err: any) {
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between border-b pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Current State: <span className="font-bold text-emerald-900">{org?.status}</span>
              </p>
            </div>
            ...
            {org?.status !== 'VERIFIED' && (
              ...
              >
                Complete Accreditation (Mark VERIFIED)
              </button>
            )}
          </div>
      ...
      {activeSubTab === 'trusted_devices' && (
        <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {d.status === 'TRUSTED' ? (
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        >
                          {processingDeviceId === d.id ? 'Restoring...' : 'Re-Trust'}
                        </button>
                        <button
          ...
          <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
            <h4 className="text-sm font-bold text-slate-900">Bind Current Physical Workstation</h4>
            <form onSubmit={handleRegisterDevice} className="flex gap-3 text-xs">
              <input
                type="text"
                value={deviceName}
                onChange={e => setDeviceName(e.target.value)}
                placeholder="e.g. Controller Office Primary Workstation"
                required
