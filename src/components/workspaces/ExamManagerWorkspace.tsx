...
// };
import React, { useState, useEffect } from 'react';
import { FolderLock, PlusCircle, HelpCircle, Cpu, Layers, Building2, Lock, CheckCircle2, AlertTriangle, Clock, Sparkles, FileCheck, Upload, KeyRound, RotateCcw, Languages, UserCheck, CheckSquare, Square, FileUp, Edit3, Trash2, ArrowRight, Search, Filter, Check, X, FileText, Activity, Send, Users, } from 'lucide-react';
...
export const ExamManagerWorkspace: React.FC<ExamManagerWorkspaceProps> = ({ currentUser, activeSubTab, onRefresh, onSelectSubTab, }) => {
    const [org, setOrg] = useState<Organization | null>(null);
    const [examinations, setExaminations] = useState<Examination[]>([]);
    const [questions, setQuestions] = useState<Question[]>([]);
    ...
    const [universityDraftFiles, setUniversityDraftFiles] = useState<Array<{
        id: string;
        name: string;
        size: string;
        uploadedAt: string;
    ...
    // Update a field in an extracted question
    const updateExtractedQuestion = (id: string, updates: Partial<ExtractedQuestion>) => {
        setExtractedQuestions(prev => prev.map(q => (q.tempId === id ? { ...q, ...updates } : q)));
    };
    // Remove question from extraction list
    const removeExtractedQuestion = (id: string) => {
        setExtractedQuestions(prev => prev.filter(q => q.tempId !== id));
    ...
    const handleImportExtracted = async () => {
        ...
        try {
            ...
            setStatusMessage({
                type: 'success',
                text: `Successfully imported ${res.createdCount} questions into the secure question bank${extractAssignSmeId ? ' and assigned to SME' : ''}${extractAssignTranslatorId ? ` and assigned for ${extractAssignTargetLanguage} translation` : ''}.`,
            });
            // Clear extracted list on success
            setExtractedQuestions([]);
            setSelectedExtractedIds(new Set());
    ...
    const handleAnalyzeTheoryPattern = async () => {
        if (!selectedExamId || !refDraftText)
            return;
        setAnalyzingPattern(true);
        try {
            const res = await api.analyzeTheoryPattern(selectedExamId, refDraftText);
    ...
    const handleConfirmPattern = async () => {
        if (!selectedExamId || !extractedPattern)
            return;
        try {
            const res = await api.confirmPattern(selectedExamId, {
                theory_pattern_json: extractedPattern,
    ...
    return (<div className="space-y-6">
      {/* Organization Verification Gate Notification */}
      {org?.status !== 'VERIFIED' && (<div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs flex items-start gap-3 shadow-xs">
          <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5"/>
          <div className="space-y-1">
            <h4 className="font-bold">Organization Verification Required</h4>
            <p className="text-amber-800 leading-relaxed">
      ...
      {statusMessage && (<div className={`p-3 rounded-lg text-xs flex items-center gap-2 ${statusMessage.type === 'success'
                ? 'bg-emerald-50 text-emerald-800 border border-emerald-200'
                : 'bg-rose-50 text-rose-800 border border-rose-200'}`}>
      ...
      {/* DASHBOARD */}
      {activeSubTab === 'dashboard' && (<div className="space-y-6">
          <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3">
              <div>
            ...
            {/* 4 Interactive Metric Cards */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <button type="button" onClick={() => setDashboardCardFilter('ALL')} className={`p-4 rounded-xl border text-left transition-all cursor-pointer ${dashboardCardFilter === 'ALL'
              ...
              <button type="button" onClick={() => setDashboardCardFilter('READY')} className={`p-4 rounded-xl border text-left transition-all cursor-pointer ${dashboardCardFilter === 'READY'
                ? 'bg-slate-900 text-white border-slate-900 ring-2 ring-slate-900/20 shadow-xs'
                : 'bg-slate-50 border-slate-200 text-slate-900 hover:bg-slate-100 hover:border-slate-300'}`}>
              ...
              <button type="button" onClick={() => setDashboardCardFilter('VERIFIED')} className={`p-4 rounded-xl border text-left transition-all cursor-pointer ${dashboardCardFilter === 'VERIFIED'
              ...
              <button type="button" onClick={() => setDashboardCardFilter('QUARANTINED')} className={`p-4 rounded-xl border text-left transition-all cursor-pointer ${dashboardCardFilter === 'QUARANTINED'
          ...
          <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
            ...
            {allExaminationsError ? (<div className="p-6 rounded-xl border border-rose-200 bg-rose-50 text-rose-800 text-sm space-y-3">
                <p>Unable to load examinations.</p>
                <button type="button" onClick={() => loadData()} className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-bold text-xs">
                  Retry
                </button>
              </div>) : loading ? (<div className="p-8 text-center text-xs text-slate-500">Loading examinations...</div>) : (<>
                <div className="flex flex-col lg:flex-row gap-3">
                  <div className="relative flex-1">
                    <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2"/>
                    <input type="text" value={allExamsSearch} onChange={e => setAllExamsSearch(e.target.value)} placeholder="Search examinations..." className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-300 bg-slate-50 text-slate-900 text-xs placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-600/20"/>
                  </div>

                  <select value={allExamsStatusFilter} onChange={e => setAllExamsStatusFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-slate-300 bg-slate-50 text-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-600/20">
                    <option value="ALL">All Status</option>
                    {statusOptions.map(status => (<option key={status} value={status}>{status}</option>))}
                  </select>

                  <select value={allExamsCategoryFilter} onChange={e => setAllExamsCategoryFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-slate-300 bg-slate-50 text-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-600/20">
                    <option value="ALL">All Categories</option>
                    {categoryOptions.map(category => (<option key={category} value={category}>{category}</option>))}
                  </select>
                </div>

                {filteredAllExaminations.length === 0 ? (<div className="p-8 rounded-xl border border-dashed border-slate-300 bg-slate-50 text-center space-y-3">
                    ...
                    <button type="button" onClick={() => onSelectSubTab?.('create_examination')} className="px-4 py-2 bg-emerald-900 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs">
                      Create Examination
                    </button>
                  </div>) : (<div className="space-y-3">
                    {filteredAllExaminations.map(ex => (<div key={ex.id} className="p-4 rounded-xl border border-slate-200 bg-slate-50/70 space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-base font-bold text-slate-900">{ex.name}</span>
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${ex.exam_type === 'MCQ'
                        ...
                        <div className="flex justify-end">
                          <button type="button" onClick={() => handleOpenExamination(ex.id)} className="px-4 py-2 bg-emerald-900 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs">
                            Open Examination
                          </button>
                        </div>
                      </div>))}
          ...
          {dashboardCardFilter !== 'ALL' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  <FolderLock className="w-4 h-4 text-emerald-900"/>
                  <span>
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto">
                {questions
                    .filter(q => {
                    if (dashboardCardFilter === 'VERIFIED')
                        return q.status === 'VERIFIED' || q.status === 'ELIGIBLE_FOR_PAPER';
                    if (dashboardCardFilter === 'QUARANTINED')
                        return q.status === 'QUARANTINED' || q.status === 'COMPROMISED';
                    return true;
                })
                    .map(q => (<div key={q.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200 flex items-start justify-between gap-3 text-xs">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-slate-800">{q.id}</span>
                          <span className="font-bold text-slate-900">{q.subject}</span>
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold shrink-0 ${q.status === 'VERIFIED' || q.status === 'ELIGIBLE_FOR_PAPER'
                        ? 'bg-emerald-100 text-emerald-900 border border-emerald-200'
                        : q.status === 'QUARANTINED' || q.status === 'COMPROMISED'
                            ? 'bg-rose-100 text-rose-900 border border-rose-200'
                            : 'bg-amber-100 text-amber-900 border border-amber-200'}`}>
      ...
      {/* ALL EXAMINATIONS */}
      {activeSubTab === 'all_examinations' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3">
                ...
                const typeBadgeClass = type === 'MCQ'
                    ...
                    : type === 'THEORY'
                        ...
                        : type === 'MIXED'
                            ? 'bg-teal-100 text-teal-900 border-teal-200'
                            : 'bg-indigo-100 text-indigo-900 border-indigo-200';
                return (<div key={ex.id} className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-3 text-xs">
      ...
      {/* CREATE EXAMINATION */}
      {activeSubTab === 'create_examination' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
          ...
          <form onSubmit={handleCreateExam} className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="sm:col-span-2">
              <label className="block text-slate-700 font-bold mb-1">Official Examination Name *</label>
              <input type="text" value={examName} onChange={e => setExamName(e.target.value)} placeholder="e.g. National Computer Science & Security Entrance Exam 2026" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div>
              <label className="block text-slate-700 font-bold mb-1">Subject / Domain *</label>
              <input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. Computer Science & Applied Cryptography" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div>
              <label className="block text-slate-700 font-bold mb-1">Examination Category</label>
              <select value={category} onChange={e => setCategory(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden">
            ...
            <div>
              <label className="block text-slate-700 font-bold mb-1">Examination Paper Type *</label>
              <select value={examType} onChange={e => setExamType(e.target.value as any)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden font-medium">
                <option value="MCQ">MCQ (Multiple Choice Questions - OMR / CBT)</option>
                <option value="THEORY">THEORY (Subjective / Descriptive Pattern)</option>
                <option value="MIXED">MIXED / HYBRID (MCQ + Descriptive Sections)</option>
                <option value="PRACTICAL_CODING">PRACTICAL / CODING (Lab & Assessment)</option>
            ...
            <div>
              <label className="block text-slate-700 font-bold mb-1">Scheduled Examination Date *</label>
              <input type="date" value={examDate} onChange={e => setExamDate(e.target.value)} required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div>
              <label className="block text-slate-700 font-bold mb-1">Examination Start Time *</label>
              <input type="time" value={examTime} onChange={e => setExamTime(e.target.value)} required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div>
              <label className="block text-slate-700 font-bold mb-1">Authorized Time-Lock Unlock *</label>
              <input type="time" value={unlockTime} onChange={e => setUnlockTime(e.target.value)} required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div>
              <label className="block text-slate-700 font-bold mb-1">Total Marks</label>
              <input type="number" value={totalMarks} onChange={e => setTotalMarks(Number(e.target.value))} required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div>
              <label className="block text-slate-700 font-bold mb-1">Target Question Count</label>
              <input type="number" value={totalQuestions} onChange={e => setTotalQuestions(Number(e.target.value))} required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
            </div>

            <div className="sm:col-span-2 pt-2">
              <button type="submit" disabled={org?.status !== 'VERIFIED'} className="px-6 py-2.5 bg-emerald-900 hover:bg-emerald-800 disabled:opacity-40 text-white rounded-lg font-bold text-xs shadow-xs">
      ...
      {activeSubTab === 'question_workflow' && (<div className="space-y-6">
          {/* Navigation Pill Bar for Question Workflow */}
          <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-xs">
            <div className="flex items-center gap-2">
              <button onClick={() => setQuestionWorkflowTab('extraction')} className={`px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 transition-all ${questionWorkflowTab === 'extraction'
                ? 'bg-emerald-900 text-white shadow-xs'
                : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                <FileUp className="w-4 h-4 text-emerald-400"/>
                <span>PDF Question Paper Extractor</span>
                {extractedQuestions.length > 0 && (<span className="px-1.5 py-0.5 rounded-full text-[10px] bg-emerald-700 text-white font-mono">
                    {extractedQuestions.length}
                  </span>)}
              </button>

              <button onClick={() => setQuestionWorkflowTab('manual')} className={`px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 transition-all ${questionWorkflowTab === 'manual'
                ? 'bg-emerald-900 text-white shadow-xs'
                : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                <PlusCircle className="w-4 h-4"/>
              ...
              <button onClick={() => setQuestionWorkflowTab('matrix')} className={`px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 transition-all ${questionWorkflowTab === 'matrix'
                ? 'bg-emerald-900 text-white shadow-xs'
                : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
          ...
          {questionWorkflowTab === 'extraction' && (<div className="space-y-6">
              <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3">
                  <div>
                    <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                {isUniversityWorkflow && (<div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      {universityDraftFiles.length === 0 ? (<div className="text-xs text-slate-500 border border-dashed border-slate-300 rounded-lg p-3 bg-white">No draft PDFs uploaded yet.</div>) : (universityDraftFiles.map((file, index) => (<div key={file.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs">
                            <div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <label className="inline-flex cursor-pointer items-center px-4 py-2 rounded-lg bg-slate-800 text-white text-xs font-bold hover:bg-slate-700 disabled:opacity-50" aria-disabled={universityDraftFiles.length >= 3}>
                        ...
                        Add PDF
                        <input type="file" accept=".pdf" onChange={handleUniversityDraftUpload} className="hidden" disabled={universityDraftFiles.length >= 3}/>
                      </label>

                      <span className="text-[11px] text-slate-500">Maximum 3 PDFs allowed for University Exam.</span>
                    </div>

                    {draftUploadError && (<div className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] text-rose-700">{draftUploadError}</div>)}
                  </div>)}

                {!isUniversityWorkflow && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <div>
                      <label className="block text-slate-700 font-bold mb-1">Target Subject / Discipline</label>
                      <input type="text" value={paperSubject} onChange={e => setPaperSubject(e.target.value)} placeholder="e.g. Physics / Mathematics / Computer Science" className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
                    </div>

                    <div>
                      <label className="block text-slate-700 font-bold mb-1">Examination Category</label>
                      <select value={paperCategory} onChange={e => handlePaperCategoryChange(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 focus:bg-white focus:border-emerald-800 focus:outline-hidden">
                        <option value="NEET">NEET (National Eligibility Entrance Test)</option>
                        <option value="JEE">JEE (Joint Entrance Examination)</option>
                        <option value="Competitive Exam">Competitive Exam</option>
                        <option value="TCET / CET-type Exam">TCET / CET-type Exam</option>
                ...
                {!isUniversityWorkflow && (<div className="border-2 border-dashed border-slate-300 rounded-xl p-6 bg-slate-50 hover:bg-slate-100/60 transition-colors text-center">
                    ...
                    <label className="mt-3 inline-block px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg font-bold text-xs cursor-pointer shadow-xs">
                      Browse File
                      <input type="file" accept={isLargePoolWorkflow || isCompetitiveWorkflow ? '.pdf,.txt,.docx,.json,.csv' : '.pdf,.txt,.docx,.json,.csv'} onChange={handleFileUpload} className="hidden"/>
                    </label>
                  </div>)}

                {isUniversityWorkflow && (<div className="flex items-center justify-between pt-2">
                    <button type="button" onClick={handleExtractQuestions} disabled={extracting || universityDraftFiles.length === 0} className="px-6 py-2.5 bg-emerald-900 hover:bg-emerald-800 disabled:opacity-40 text-white rounded-lg font-bold text-xs shadow-xs flex items-center gap-2">
                ...
                {!isUniversityWorkflow && (<div className="space-y-1.5 text-xs">
                    <label className="block text-slate-700 font-bold">
                      Or Paste Question Paper Content / Raw OCR Transcript:
                    </label>
                    <textarea rows={6} value={pdfText} onChange={e => setPdfText(e.target.value)} placeholder={`e.g.
...
Marks: 10`} className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 font-mono text-xs focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
                  </div>)}

                {!isUniversityWorkflow && (<div className="flex items-center justify-between pt-2">
                    <button type="button" onClick={handleExtractQuestions} disabled={extracting || (!pdfText.trim() && !pdfFileData)} className="px-6 py-2.5 bg-emerald-900 hover:bg-emerald-800 disabled:opacity-40 text-white rounded-lg font-bold text-xs shadow-xs flex items-center gap-2">
                      <Sparkles className="w-4 h-4 text-emerald-300"/>
                      <span>{extracting ? 'Extracting Questions with AI...' : 'Run AI Question Extraction'}</span>
                    </button>

                    {(pdfText || pdfFileName) && (<button type="button" onClick={() => {
                            setPdfText('');
                            setPdfFileData('');
                            setPdfFileName('');
                            setExtractedQuestions([]);
              ...
              {extractedQuestions.length > 0 && (<div className="space-y-4">
                  {/* Summary & Filter Bar */}
                  <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
                    <div>
                      ...
                      {extractionSummary && (<p className="text-xs text-emerald-800 mt-1 pl-7">{extractionSummary}</p>)}
                    </div>

                    {/* Selection Controls */}
                    <div className="flex items-center gap-1.5 text-xs pl-7 sm:pl-0">
                      <button type="button" onClick={() => selectAllExtracted('ALL')} className="px-2.5 py-1 bg-white hover:bg-emerald-100 text-emerald-900 border border-emerald-300 rounded font-semibold text-[11px]">
                        Select All ({extractedQuestions.length})
                      </button>
                      <button type="button" onClick={() => selectAllExtracted('MCQ')} className="px-2.5 py-1 bg-white hover:bg-emerald-100 text-emerald-900 border border-emerald-300 rounded font-semibold text-[11px]">
                        MCQ Only
                      </button>
                      <button type="button" onClick={() => selectAllExtracted('THEORY')} className="px-2.5 py-1 bg-white hover:bg-emerald-100 text-emerald-900 border border-emerald-300 rounded font-semibold text-[11px]">
                        Theory Only
                      </button>
                      <button type="button" onClick={deselectAllExtracted} className="px-2.5 py-1 bg-white hover:bg-rose-50 text-rose-700 border border-rose-200 rounded font-semibold text-[11px]">
                        Clear
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-6 rounded-xl bg-slate-900 text-white border border-slate-800 shadow-md space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                      ...
                      <div>
                        ...
                        <select value={extractAssignSmeId} onChange={e => setExtractAssignSmeId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-emerald-500 focus:outline-hidden">
                          <option value="">-- No SME Assignment (Direct Pool) --</option>
                          {smes.map(s => (<option key={s.id} value={s.id}>
                              {s.full_name} ({s.email})
                            </option>))}
                        </select>
                      ...
                      <div>
                        ...
                        <select value={extractAssignTranslatorId} onChange={e => setExtractAssignTranslatorId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-emerald-500 focus:outline-hidden">
                          <option value="">-- No Translator Assignment --</option>
                          {translators.map(t => (<option key={t.id} value={t.id}>
                              {t.full_name} ({t.email})
                            </option>))}
                        </select>
                        <span className="text-[10px] text-slate-400 mt-0.5 block">
                      ...
                      <div>
                        ...
                        <select value={extractAssignTargetLanguage} onChange={e => setExtractAssignTargetLanguage(e.target.value)} disabled={!extractAssignTranslatorId} className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-emerald-500 focus:outline-hidden disabled:opacity-50">
                          {SUPPORTED_TRANSLATION_LANGUAGES.map(l => (<option key={l.code} value={l.code}>
                              {l.label}
                            </option>))}
                        </select>
                      </div>
                    ...
                    <div className="text-xs">
                      ...
                      <input type="text" value={extractAssignNotes} onChange={e => setExtractAssignNotes(e.target.value)} placeholder="e.g. Ensure strict adherence to CBSE Class 12 Syllabus 2026 and verify MCQ answer derivations." className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:border-emerald-500 focus:outline-hidden"/>
                    </div>

                    <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-800">
                      <button type="button" onClick={handleImportExtracted} disabled={importingBatch || selectedExtractedIds.size === 0} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded-lg font-bold text-xs shadow-md flex items-center gap-2 transition-all">
                  ...
                  <div className="space-y-3">
                    {extractedQuestions.map((q, idx) => {
                        ...
                        return (<div key={q.tempId} className={`p-4 rounded-xl border transition-all text-xs space-y-3 ${isSelected
                                ? 'bg-white border-emerald-400 ring-2 ring-emerald-500/10 shadow-xs'
                                : 'bg-slate-50/70 border-slate-200 opacity-70'}`}>
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3">
                              <button type="button" onClick={() => toggleExtractedSelection(q.tempId)} className="text-emerald-800 hover:text-emerald-900">
                                {isSelected ? (<CheckSquare className="w-5 h-5 text-emerald-700"/>) : (<Square className="w-5 h-5 text-slate-400"/>)}
                              </button>
                              <span className="font-bold text-slate-900 text-xs">
                                Question #{idx + 1}
                              </span>
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${q.question_type === 'MCQ'
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button type="button" onClick={() => removeExtractedQuestion(q.tempId)} className="p-1 text-slate-400 hover:text-rose-600 rounded" title="Remove from batch">
                          ...
                          <div>
                            <textarea rows={2} value={q.content_text} onChange={e => updateExtractedQuestion(q.tempId, { content_text: e.target.value })} className="w-full px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-300 text-slate-900 text-xs focus:bg-white focus:border-emerald-800 focus:outline-hidden"/>
                          </div>

                          {/* MCQ Options */}
                          {q.question_type === 'MCQ' && q.options && q.options.length > 0 && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-slate-100">
                              {q.options.map((opt, optIdx) => (<div key={optIdx} className="flex items-center gap-2">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 pt-1 border-t border-slate-100 text-[11px]">
                            <div>
                              ...
                              <input type="text" value={`${q.subject} - ${q.topic}`} onChange={e => {
                            ...
                            <div>
                              ...
                              <select value={q.difficulty} onChange={e => updateExtractedQuestion(q.tempId, { difficulty: e.target.value as any })} className="w-full px-2 py-0.5 rounded bg-slate-50 border border-slate-300 text-slate-900 text-[11px]">
                            ...
                            <div>
                              ...
                              <input type="text" value={q.correct_answer} onChange={e => updateExtractedQuestion(q.tempId, { correct_answer: e.target.value })} className="w-full px-2 py-0.5 rounded bg-slate-50 border border-slate-300 text-slate-900 font-bold text-[11px]"/>
                            </div>

                            <div>
                              <span className="text-slate-500 block">Marks:</span>
                              <input type="number" value={q.marks} onChange={e => updateExtractedQuestion(q.tempId, { marks: Number(e.target.value) })} className="w-full px-2 py-0.5 rounded bg-slate-50 border border-slate-300 text-slate-900 text-[11px]"/>
                            </div>
                          </div>
                        </div>);
                    })}
          ...
          {questionWorkflowTab === 'manual' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
              <h3 className="text-base font-bold text-slate-900 border-b pb-2">
              ...
              <form onSubmit={handleCreateQuestion} className="space-y-4 text-xs">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <input type="text" value={qSubject} onChange={e => setQSubject(e.target.value)} placeholder="e.g. Computer Science" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
                  </div>

                  <div>
                    <label className="block text-slate-700 font-bold mb-1">Topic</label>
                    <input type="text" value={qTopic} onChange={e => setQTopic(e.target.value)} placeholder="e.g. Cryptography" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
                  </div>

                  <div>
                    <label className="block text-slate-700 font-bold mb-1">Difficulty</label>
                    <select value={qDifficulty} onChange={e => setQDifficulty(e.target.value as any)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900">
                      <option value="EASY">EASY</option>
                      <option value="MEDIUM">MEDIUM</option>
                      <option value="HARD">HARD</option>
                    </select>
                ...
                <div>
                  ...
                  <textarea rows={3} value={qContent} onChange={e => setQContent(e.target.value)} placeholder="Enter complete question statement with mathematical or theoretical criteria..." required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
                </div>

                {/* AI Similarity & Duplicate Check Action */}
                <div className="flex items-center gap-3">
                  <button type="button" onClick={handleCheckAiSimilarity} disabled={aiChecking || !qContent} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-200 rounded-lg font-bold text-xs">
                    <Sparkles className="w-3.5 h-3.5 text-emerald-700"/>
                    <span>{aiChecking ? 'Analyzing Pool...' : 'AI Duplicate & Similarity Check'}</span>
                  </button>

                  {aiCheckResult && (<span className="text-[11px] text-emerald-800 font-semibold bg-emerald-50 px-2.5 py-1 rounded border border-emerald-200">
                      Similarity Score: {aiCheckResult.similarityScore || '0.04'} (No Conflict Detected)
                    </span>)}
                </div>

                {examType === 'MCQ' && (<div className="grid grid-cols-2 gap-3 pt-2 border-t">
                    {qOptions.map((opt, i) => (<div key={i}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                  <div>
                    ...
                    <input type="text" value={qCorrectAnswer} onChange={e => setQCorrectAnswer(e.target.value)} placeholder="e.g. B or Derivation" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
                  </div>

                  <div>
                    <label className="block text-slate-700 font-bold mb-1">Marks</label>
                    <input type="number" value={qMarks} onChange={e => setQMarks(Number(e.target.value))} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
                  </div>

                  <div>
                    <label className="block text-slate-700 font-bold mb-1">Assign to SME for Verification</label>
                    <select value={assignedSmeId} onChange={e => setAssignedSmeId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900">
                      <option value="">-- Select SME Verifier --</option>
                      {smes.map(s => (<option key={s.id} value={s.id}>{s.full_name} ({s.email})</option>))}
                    </select>
                  </div>
                </div>

                <button type="submit" className="px-5 py-2.5 bg-emerald-900 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs shadow-xs">
          ...
          {/* TAB 3: ASSIGNMENT & VERIFICATION MATRIX */}
          {questionWorkflowTab === 'matrix' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3">
                <div>
                  <h3 className="text-base font-bold text-slate-900">
              ...
              {assignments.length === 0 ? (<p className="text-xs text-slate-400 p-6 text-center bg-slate-50 rounded-lg">
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {assignments.map(a => (<tr key={a.id} className="hover:bg-slate-50/80">
                          <td className="p-3 font-mono text-[11px] font-bold text-slate-800">
                            {a.question_id.substring(0, 10)}...
                          </td>
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${a.assignment_type === 'SME_REVIEW'
                            ? 'bg-blue-100 text-blue-900'
                          ...
                          <td className="p-3">
                            {a.target_language ? (<span className="px-2 py-0.5 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded font-semibold text-[10px]">
                                {a.target_language}
                              </span>) : (<span className="text-slate-400 font-mono text-[11px]">—</span>)}
                          </td>
                          <td className="p-3 text-[11px] text-slate-500 font-mono">
                            {new Date(a.assigned_at).toLocaleDateString()}
                          </td>
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${a.status === 'COMPLETED'
                            ? 'bg-emerald-100 text-emerald-800'
                            : a.status === 'REJECTED'
                                ? 'bg-rose-100 text-rose-800'
                                : 'bg-amber-100 text-amber-800'}`}>
      ...
      {/* QUESTION POOLS */}
      {activeSubTab === 'question_pools' && (<div className="space-y-6">
          <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900">Secure Question Pool Repository</h3>
              ...
              <div className="flex items-center gap-2">
                ...
                {poolSelectedIds.size > 0 && (<button type="button" onClick={() => setPoolBulkModalOpen(true)} className="px-4 py-1.5 bg-emerald-900 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs shadow-xs flex items-center gap-1.5">
                    <Send className="w-3.5 h-3.5"/>
                    <span>Assign Selected ({poolSelectedIds.size})</span>
                  </button>)}
              </div>
            ...
            {/* Bulk Assignment Modal */}
            {poolBulkModalOpen && (<div className="p-4 rounded-xl bg-slate-900 text-white border border-slate-800 space-y-3 text-xs shadow-md">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <h4 className="font-bold text-white flex items-center gap-2">
                    <Users className="w-4 h-4 text-emerald-400"/>
                    <span>Bulk Assign {poolSelectedIds.size} Questions</span>
                  </h4>
                  <button onClick={() => setPoolBulkModalOpen(false)} className="text-slate-400 hover:text-white">
                    ✕
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <select value={poolAssignType} onChange={e => {
                  ...
                  <div>
                    ...
                    <select value={poolAssignUserId} onChange={e => setPoolAssignUserId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white">
                      <option value="">-- Select Member --</option>
                      {(poolAssignType === 'SME_REVIEW' ? smes : translators).map(u => (<option key={u.id} value={u.id}>
                          {u.full_name} ({u.email})
                        </option>))}
                    </select>
                  </div>

                  {poolAssignType === 'LINGUISTIC_TRANSLATION' && (<div>
                      <label className="block text-slate-300 font-bold mb-1">Target Language</label>
                      <select value={poolAssignTargetLanguage} onChange={e => setPoolAssignTargetLanguage(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white">
                        {SUPPORTED_TRANSLATION_LANGUAGES.map(l => (<option key={l.code} value={l.code}>
                            {l.label}
                          </option>))}
                      </select>
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1">Instructions / Notes</label>
                  <input type="text" value={poolAssignNotes} onChange={e => setPoolAssignNotes(e.target.value)} placeholder="Instructions for verifier..." className="w-full px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-white"/>
                </div>

                <div className="flex justify-end gap-2 pt-2">
                  <button onClick={() => setPoolBulkModalOpen(false)} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded font-bold">
                    Cancel
                  </button>
                  <button onClick={handleExecutePoolBulkAssign} disabled={poolAssigning || !poolAssignUserId} className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded font-bold">
                    {poolAssigning ? 'Assigning...' : 'Confirm Bulk Assignment'}
                  </button>
                </div>
              </div>)}
            ...
            <div className="space-y-3">
              {questions.map(q => {
                const isSelected = poolSelectedIds.has(q.id);
                return (<div key={q.id} className={`p-3.5 rounded-lg border text-xs space-y-1.5 transition-all ${isSelected
                        ? 'bg-emerald-50/60 border-emerald-400'
                        : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${q.status === 'ELIGIBLE_FOR_PAPER' || q.status === 'VERIFIED'
                        ? 'bg-emerald-100 text-emerald-800'
                        : q.status === 'QUARANTINED'
                            ? 'bg-rose-100 text-rose-800'
                            : 'bg-amber-100 text-amber-800'}`}>
      ...
      {/* BLUEPRINT & PATTERN */}
      {activeSubTab === 'blueprint_pattern' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
          <h3 className="text-base font-bold text-slate-900 border-b pb-2">
          ...
          <div className="space-y-3 text-xs">
            ...
            <textarea rows={4} value={refDraftText} onChange={e => setRefDraftText(e.target.value)} placeholder="e.g. Section A: 5 compulsory questions of 2 marks each. Section B: Attempt 3 out of 5 long theory questions of 10 marks each..." className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>

            <button onClick={handleAnalyzeTheoryPattern} disabled={analyzingPattern || !refDraftText} className="px-4 py-2 bg-emerald-900 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs shadow-xs flex items-center gap-1.5">
              <Cpu className="w-4 h-4"/>
              <span>{analyzingPattern ? 'Extracting Pattern...' : 'Extract Pattern with Gemini AI'}</span>
            </button>

            {extractedPattern && (<div className="p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-900 space-y-3">
                ...
                <button onClick={handleConfirmPattern} className="px-4 py-2 bg-emerald-800 hover:bg-emerald-700 text-white rounded-lg font-bold text-xs shadow-xs">
      ...
      {/* PAPER GENERATION */}
      {activeSubTab === 'paper_generation' && (<div className="space-y-6">
          <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-xs">
                ...
                return (<button key={t} onClick={() => setGenerationExamTypeFilter(t)} className={`px-3 py-1.5 rounded-lg font-bold text-xs transition-all ${generationExamTypeFilter === t
                        ? 'bg-emerald-900 text-white shadow-xs'
            ...
            <div className="space-y-3">
              {examinations
                ...
                return (<div key={ex.id} onClick={() => setSelectedGenExamId(ex.id)} className={`p-4 rounded-xl border cursor-pointer transition-all ${isSelected
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm text-slate-900">{ex.name}</span>
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${type === 'MCQ'
                        ? 'bg-blue-100 text-blue-900 border-blue-200'
                        : type === 'THEORY'
                            ? 'bg-amber-100 text-amber-900 border-amber-200'
                            : type === 'MIXED'
                    ...
                    }} disabled={generating || org?.status !== 'VERIFIED'} className="px-4 py-2 bg-emerald-900 hover:bg-emerald-800 disabled:opacity-40 text-white rounded-lg font-bold text-xs shadow-xs flex items-center gap-1.5">
      ...
      {/* PAPER VERSIONS */}
      {activeSubTab === 'paper_versions' && (<div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
          <h3 className="text-base font-bold text-slate-900">Encrypted Paper Version Artifacts</h3>
          <p className="text-xs text-slate-500">
            All generated examination papers are stored as ciphertext with unique version hashes and key fingerprints.
          </p>
          ...
          <div className="space-y-3">
            {examinations.map(ex => (<div key={ex.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200 text-xs flex justify-between items-center">
                <div>
                  <span className="font-bold text-slate-900">{ex.name}</span>
                  <div className="text-[10px] text-slate-500 font-mono">
                    Time Lock: {ex.unlock_time} • Plaintext status: 🔒 ENCRYPTED (Locked until unlock minute)
      ...
      {/* EXAMINATION CENTRES */}
      {activeSubTab === 'examination_centres' && (<div className="space-y-6">
          <div className="p-6 rounded-xl bg-white border border-slate-200 shadow-xs space-y-4">
            ...
            <form onSubmit={handleAddCentre} className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div>
                ...
                <select value={selectedExamId} onChange={e => setSelectedExamId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900">
                  {examinations.map(ex => (<option key={ex.id} value={ex.id}>{ex.name}</option>))}
                </select>
              </div>

              <div>
                <label className="block text-slate-700 font-bold mb-1">Centre Code</label>
                <input type="text" value={centreCode} onChange={e => setCentreCode(e.target.value)} placeholder="e.g. CTR-DELHI-101" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
              </div>

              <div>
                <label className="block text-slate-700 font-bold mb-1">Centre Name</label>
                <input type="text" value={centreName} onChange={e => setCentreName(e.target.value)} placeholder="e.g. National Institute of Technology Centre 1" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
              </div>

              <div>
                <label className="block text-slate-700 font-bold mb-1">City</label>
                <input type="text" value={centreCity} onChange={e => setCentreCity(e.target.value)} placeholder="e.g. New Delhi" required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
              </div>

              <div>
                <label className="block text-slate-700 font-bold mb-1">Max Authorized Copies</label>
                <input type="number" value={centreMaxCopies} onChange={e => setCentreMaxCopies(Number(e.target.value))} required className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-slate-900"/>
              </div>

              <div className="flex items-end">
                <button type="submit" className="w-full py-2 bg-emerald-900 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs shadow-xs">
