...
async function startServer() {
  ...
  function compilePaperPayloadForExam(db: any, exam: any, orgId: string, body: any = {}) {
    ...
    const isNeetOrMultiSubjectMCQ =
      ...
      (exam.exam_type === 'MCQ' && (body.subject_pool || (exam.subject && (exam.subject.includes('PCB') || exam.subject.includes('PCM') || exam.subject.includes('All') || exam.subject.includes('&')))));
    ...
    const now = new Date().toISOString();
    const numSetsToGenerate = isUniversityExam ? (body.num_sets || 3) : 1;
    ...
    for (let setIdx = 1; setIdx <= numSetsToGenerate; setIdx++) {
      ...
      if (isUniversityExam && numSetsToGenerate > 1) {
        // Deterministic balanced variation across university sets
      ...
      const paperPayloadObject = {
        ...
        questions: setQuestions.map((q, idx) => {
          let opts: any[] = [];
          try {
            opts = q.options_json ? JSON.parse(q.options_json) : (Array.isArray(q.options) ? q.options : []);
          } catch {
            opts = [];
          }
          return {
            ...
            questionType: q.question_type || (opts.length > 0 ? 'MCQ' : 'THEORY'),
